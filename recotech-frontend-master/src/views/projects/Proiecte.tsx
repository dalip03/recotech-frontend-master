...
export default function Proiecte() {
    ...
    const [data, setData] = useState<{ projects: Proiect[]; favoriteProjects: Proiect[] }>({
        projects: [],
        favoriteProjects: []
    })
    ...
    const fetchProjectsData = async () => {
        try {
            ...
            const projects = hasAccess(userRole, ['ADMIN']) ? await fetchProjects() : await fetchUserProjects();
            ...
            const userProjectTypeObj = await fetchUserFavoriteProjectsByUserId(userId);
            ...
            const userProjectTypeNames = validProjectTypes
                .filter((projectType: any) => userProjectTypes.includes(projectType.id))
            ...
            const combinedProjects = [...new Set([...validProjects, ...extendedProjects])];
    ...
    const handleFavorite = async (id: string) => {
        const isFavorite = data.favoriteProjects.some((project) => project.id === id);
        if (isFavorite) {
    ...
    const columns: ColumnDef<Proiect>[] = [
        ...
        },
        {
            ...
            accessorKey: 'tasks',
            cell: ({ row }) => {
                let projectTasksPercentage = row.original.completedTaskCount / row.original.taskCount * 100;
                    ...
                    rowActions.push(
                        {
                            ...
                            label: data.favoriteProjects.some((project) => project.id === row.original.id)
                                ? t("Unfavorite")
                                : t("Favorite"),
    ...
    return (
        <div>
            <div>
                <h3 className="text-xl font-semibold mb-4">{t("Projects")}</h3>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <Card
                    ...
                    className="hover:shadow-lg transition duration-150 ease-in-out rounded-2xl"
                >
                    <h5 className="text-4xl font-bold">{t("Projects in progress")}</h5>
                    <p className="mt-4 text-4xl font-bold">
                        {' '}
                ...
                <Card
                    ...
                    className="hover:shadow-lg transition duration-150 ease-in-out rounded-2xl"
                >
                    <h5 className="text-4xl font-bold">{t("Finished Projects")}</h5>
                    <p className="mt-4 text-4xl font-bold">
