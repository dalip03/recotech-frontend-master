...
const validationSchema = Yup.object().shape({
    ...
    assignedTo: Yup.array()
        .of(Yup.string())
        .required('Asignat către este necesar'), // Validate as array of strings
    ...
    dueDate: Yup.string()
        ...
        .matches(
            /^\d{4}-\d{2}-\d{2}$/,
            'Formatul datei este invalid (AAAA-LL-ZZ)',
        ),
})

export default function ModalSarcini({
    isOpen,
    onClose,
    handleSubmit,
    data,
    ...
    const initializeFormValues = (projects: any) => {
        const initProjectType =
            projectId != null
                ? projects.find((project: any) => project.id == projectId)?.type
                : ''
        const initialValues: FormValues = {
            ...
            assignedTo: data?.assignedTo
                ? data.assignedTo.map((user: any) => user.id)
                : [],
            projectId:
                projectId != null
                    ? parseInt(projectId)
                        ...
                        <div className="max-h-[70vh] w-full overflow-y-auto space-y-5 p-5">
                            <Form className="p-4">
                                                        ...
                                                        const options = [
                                                            {
                                                                value: -1,
                                                                label: 'Unassigned',
                                                            }, // Add "Unassigned" as a dropdown option
                                                            ...modalData.users.map(
                                                                (
                                                                    user: any,
                                                                ) => ({
                                        ...
                                        <Field name="projectType">
                                            {({ field, form }: FieldProps) => (
                                                <Select
                                                    {...field}
                                                    options={
                                                        modalData.projectTypes
                                                    }
                                                    value={modalData.projectTypes.find(
                                                        (option: any) =>
                                                            option.value ===
                                                            field.value,
                                                    )}
                                ...
                                <div className="flex justify-end mt-6">
                                    ...
                                    {hasAccess(user.authority as UserRole, [
                                        'ADMIN',
                                        'OPERATOR',
                                        'VANZATOR',
                                    ]) && (
